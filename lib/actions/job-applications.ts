...
//! this is server action , it is a special type of function that is only run on the server and can be called directly from the client components , with it we can access db and do alot of stuff without exposing any of that to the client 
// ! api routes do the same thing but will return to it later
...
export async function createJobApplication(data: JobApplicationData) {
  ...
  const maxOrder = (await JobApplication.findOne({ columnId })
    .sort({ order: -1 })
    .select("order")
    .lean()) as { order: number } | null; // lean here to get a plain JS object so we can easily access the order field
  ...
  //!  this will invalidate the cache for the /dashboard page and cause it to re-render with the new data , because we used the cashe in the dashboard page , but before everything add in next,confif file the code to enable caching for components
